...
import ReportDetails from './detail-form/ReportDetail.js';
import ReportUpdate from './update-form/ReportUpdate.js';
...
const Report = () => {
    ...
    const [u, setU] = useState(state);

    useEffect(() => {
        if (state && state.name) console.log('we got state..', state);
        else {
            console.log('no state here..');
        }
    }, []);
                            ...
                            <IconButton
                                ...
                                onClick={() => {
                                    if (operationCheck('Patients', 'update') === 1) {
                                        ...
                                        // const encodedData = crypt.encryptData(data);
                                        // localStorage.setItem('UpdateReport', JSON.stringify(encodedData));
                ...
                try {
                    ...
                    for (let i = 0; i < Object.values(data.patient).length; i++) {
                        const n = {
                            ...
                            patient_gender: data.patient[i].patient_gende.toLowerCase() === 'Male' ? 'M' : 'F',
                            patient_email: data.patient[i].patient_email,
                            patient_mobile: data.patient[i].patient_mobile,
                            patient_mobile_alt: data.patient[i].patient_mobile_alt,
                            patient_address: data.patient[i].patient_address,
        ...
        <>
            {loader && (
                <div>
                    ...
                    {details && <ReportDetails setDetails={setDetails} patientsDetail={patientsDetail} />}
