...
const User = () => {
    ...
    const ToggleComponent = ({ params }) => {
        const switchActiveStatus = () => {
            if (operationCheck('User', 'update') === 1) {
                    ...
                    <>
                        <div style={{ display: 'flex' }}>
                            ...
                            <IconButton
                                ...
                                color="error"
                                onClick={() => {
                                    if (operationCheck('User', 'delete') === 1) {
                                        const data = {
                                            in_institute_id: params.row.institute_id,
                                            in_center_id: params.row.center_id,
                                            in_center_name: params.row.center_name,
    ...
    useEffect(() => {
        const fetchList = async () => {
            try {
                const institute = { in_institute_id: user_info.out_institute_id, in_is_doctor: 0, in_center_id: user_info.out_center_id };
    ...
    useEffect(() => {
        const fetchList = async () => {
            try {
                ...
                try {
                    // const institute = { in_institute_id: user_info.out_institute_id, in_user_id: user_info.out_userid, in_is_doctor: 0 };
                    const institute = {
    ...
    useEffect(() => {
        const fetchList = async () => {
            try {
                // const institute = { in_institute_id: user_info.out_institute_id, in_is_doctor: 0 };
                const institute = { in_institute_id: user_info.out_institute_id, in_is_doctor: 0, in_center_id: user_info.out_center_id };
        ...
        <>
            {loader && (
                <div>
                    ...
                    {add && <AddUser addDoctorHandler={addDoctorHandler} setAdd={setAdd} error={error} setError={setError} />}

                    {remove && (
                        <Delete setDeleteConfirmation={setDeleteConfirmation} error={error} setError={setError} setRemove={setRemove} />
                    )}
                    {details && <UserDetails setDetails={setDetails} doctorDetail={doctorDetail} />}
                    {update && (
                        <UpdateUser
